...
## Тесты 
`backend/pytest.ini` заменяет переменные окружения теми, что вы опишете 
в backend/.env.test , что крайне удобно для локально тестирования. 
### Важно: 
Запускать `pytest` нужно именно находясь в директории `backend`, иначе 
конфигурации `backend/pytest.ini` просто не применятся. Выскочит предупреждение, что порты хранилищ не прошли тесты, сигнализируя о том, что используются "боевые", а не локальные хранилища.  
