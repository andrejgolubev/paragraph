...
Также есть возможность локального запуска через nginx. Для этого потребуется поднять стэк из `docker-compose-local.yaml` в корне проекта и в `frontend/.env` поставить `APP__LOCAL_NGINX=true`, чтобы фронт и бэк знали, что Вы такое задумали.
